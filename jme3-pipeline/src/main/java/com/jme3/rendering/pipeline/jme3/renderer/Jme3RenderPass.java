...
public abstract class Jme3RenderPass<T extends Jme3RenderPass> extends RenderPass<T> {
    ...
    @Override
    public T resetOutColors() {
        for(int i=0;;i++)  if(useOutput(RenderOutput.Color +i,null)==null)break; //reset
        return (T) this;
    }

    @Override
    public T resetOutDepth(){
        useOutput(RenderOutput.Depth,null);
        return (T) this;
    }
    ...
    @Override
   public T outColor(int i,Texture color) {
        useOutput(RenderOutput.Color + (i), color);    
        return (T) this;
    }
    ...
    protected void onOutput(Pipeline pipeline, Object key, Object value) {
        if (key instanceof Number) {
            ...
            if (keyn == RenderOutput.Depth) {
                ...
                if(outColors.size() < tid){
                    if(logger.isLoggable(java.util.logging.Level.WARNING    )){
                        logger.log(Level.WARNING, "Trying to set output target n{0}, missing targets in the sequence will be filled with nulls.", tid);
                    }
                }
